...
def load_and_process_data(data_path='0', delim=','):
    if data_path == '0':
...
def generate_graphs(data, confusion_mat):
    ...
    PCA3_dataframe = pd.DataFrame(data=PCA3, columns=['Principle Component 1', 'Principle Component 2', 'Principle Component 3'])
    ...
    for label in range(min_class_number, max_class_number + 1):
        ...
        correct_PCA.append(PCA3_dataframe[(PCA3_dataframe[class_column_name] == label) & (PCA3_dataframe['guess'] == 1)])
        incorrect_PCA.append(PCA3_dataframe[(PCA3_dataframe[class_column_name] == label) & (PCA3_dataframe['guess'] == 0)])
...
if __name__ == '__main__':
    ...
    # Import the dataset
    dataset_path = Path(
        r'C:\Users\David Hunter\OneDrive\Northeastern Classes\Graduate\EECE5644MachineLearning\Homework1\winequality-white.csv')  # Path to the dataset
    dataset, means, covariances, reg_covariances, class_priors = load_and_process_data(dataset_path, ';')
